    ...
    if ($user_role === 'admin') {
        ...
        if ($result_users->num_rows > 0) {
            ...
            while ($row = $result_users->fetch_assoc()) {
                echo "<li>ID: " . $row['id'] . ", Имя пользователя: " . $row['username'] . " 
                      <button onclick='editUser(" . $row['id'] . ")'>Изменить</button>
                      <button onclick='confirmDelete(" . $row['id'] . ")'>Удалить</button></li>";
            }
            echo "</ul>";
            echo "<button onclick='showAddUserForm()'>Добывать пользователя</button>";
            echo "</div>";
        } else {
...
<body>
    <div class="container">
        ...
        <button type="submit"><a href="evpatoria.php">Перейти главную страницу</a></button>
